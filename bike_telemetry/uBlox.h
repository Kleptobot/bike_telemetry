...
namespace Ublox {
  ...
  param PM2_settings[]={
    {"maxStartupStateDur",0,255,0,{false,false}},
    {"extintSel",0,1,0,{false,false}},
    {"extintWake",0,1,0,{false,false}},
    {"extintBackup",0,1,0,{false,false}},
    {"limitPeakCurr",0,1,0,{false,false}},
    {"waitTimeFix",0,1,0,{false,false}},
    {"updateRTC",1,1,0,{false,false}},
    {"updateEPH",1,1,0,{false,false}},
    {"doNotEnterOff",0,1,0,{false,false}},
    {"mode",0,1,0,{false,false}},
    {"updatePeriod",60000,4294967295,0,{false,false}},
    {"searchPeriod",10,4294967295,0,{false,false}},
    {"gridOffset",0,4294967295,0,{false,false}},
    {"onTime",2,65535,0,{false,false}},
    {"minAcqTime",0,65535,0,{false,false}}
  };
  menu_item back,save;
  ...
  std::vector<menu_item*> menuVector = [] {
    ...
    for (param& setting : PM2_settings) {
  ...
  void drawSettingsItem(int16_t x, int16_t y, int index) {
    int16_t x1, y1;
    uint16_t w, h;
    uint32_t num = PM2_settings[index].val;
    const char* text = PM2_settings[index].name;
    String numText;

    if(index == items.vertLen()-1){
      drawSelectable(x,y,"Back",back.focus,back.select);
      return;
    }else if(index == items.vertLen()-2){
      drawSelectable(x,y,"Save",save.focus,save.select);
      return;
    }
    ...
    if (PM2_settings[index].state.select) {
      ...
      if (PM2_settings[index].state.focus)
  ...
  void drawGPSsettings(){
    int numSettings = items.vertLen();
    int index3 = items.vert();

    int16_t index1 = index3-2;
    if(index1 < 0)
      index1 = numSettings+index1;

    int16_t index2 = index3-1;
    if(index2 < 0)
      index2 = numSettings+index2;

    int16_t index4 = index3+1;
    if(index4 >(numSettings-1))
      index4 = index4-numSettings;

    int16_t index5 = index3+2;
    if(index5 >(numSettings-1))
      index5 = index5-numSettings;
    
    display.setTextColor(SH110X_WHITE);
    drawSettingsItem(0,0,index1);
    drawSettingsItem(0,20,index2);
    drawSettingsItem(0,40,index3);
    drawSettingsItem(0,20,index4);
    drawSettingsItem(0,20,index5);
  }

  void GPSSelect(){
    int numSettings = items.vertLen();
    int index = items.vert();
    ...
    if (!items.selected()) {
      ...
      if(UP.shortPress())
        PM2_settings[index].val++;
      else if(UP.shortHeld())
        PM2_settings[index].val++;
      else if(UP.longHeld())
        PM2_settings[index].val+=10*UP.mult();
      ...
      if(DOWN.shortPress())
        ...
        PM2_settings[index].val-=10*DOWN.mult();

      if(PM2_settings[index].val>PM2_settings[index].maxVal)
        PM2_settings[index].val=PM2_settings[index].maxVal;

      if(PM2_settings[index].val<PM2_settings[index].minVal)
        PM2_settings[index].val=PM2_settings[index].minVal;
    }
