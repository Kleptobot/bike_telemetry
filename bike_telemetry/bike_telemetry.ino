...
bool bSD_Det, bSD_Det_Prev, bSD_Det_RE, bSD_Det_FE;
...
void loadDevices() {
  if (SD.exists("devices.txt")) {
    ...
    int arrayEnd = Ublox::items.vertLen();
    for (int i=0; i<arrayEnd; i++)
    {
      Ublox::PM2_settings[i].val = jsonBuffer["GPS"][i]["val"];
    }
...
void loop() {
  ...
  if (millisDiff > 100) {
    ...
    UP.process(bUp,nCurrentTimeMillis);
    DOWN.process(bDown,nCurrentTimeMillis);
    LEFT.process(bLeft,nCurrentTimeMillis);
    RIGHT.process(bRight,nCurrentTimeMillis);
    CENTER.process(bCenter,nCurrentTimeMillis);
...
void ExitStats(){
  if(bodyStats::save.select)
  {
    ;
  }else if(bodyStats::back.select){
    ;
  }
}
...
void GUI() {
  ...
  switch (u16_state) {
    ...
    case 1:  //settings menu
      settingsMenu::drawSettings();
      settingsMenu::settingsSelect();
      ExitSettings();
      break;
    ...
    case 3:  //human info
      bodyStats::drawStats(0,0);
      bodyStats::statSelection();
      ExitStats();
      break;
    ...
    case 5:   //gps settings
      Ublox::drawGPSsettings();
      Ublox::GPSSelect();
      if (!Ublox::items.selected() && LEFT.shortPress()) {
        u16_state = 1;
        Ublox::uBlox_PM2();
        Ublox::uBlox_Save();
      }
...
void ExitSettings(){
  if (CENTER.shortPress()) {
    switch (settingsMenu::items.vert()) {
      case 0:
        u16_state = 2;
      ...
      case 1:
        ...
        break;
      case 2:
        bodyStats::nAgeDisplay = tcxLog.getAge();
        bodyStats::nMassDisplay = tcxLog.getMass();
        u16_state = 3;
        break;
      case 3:
        u16_state = 0;
...
void deviceSelection() {
  ...
  bBackSelDev = CENTER.RE() && bBackFocDev;
}
...
void saveJson(){
  ...
  int arrayEnd = std::end(Ublox::PM2_settings)-std::begin(Ublox::PM2_settings);
  for (int i=0; i<arrayEnd; i++)
  {
    GPS[i]["name"] = Ublox::PM2_settings[i].name;
    GPS[i]["val"] = Ublox::PM2_settings[i].val;
  }
...
void ExitTime() {
  if (timeMenu::save.select) {
    rtc.adjust(dtTimeDisplay);
  }
  if (timeMenu::back.select | timeMenu::save.select) {
    timeMenu::items.reset();
    u16_state = 1;
  }
}
