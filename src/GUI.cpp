...
{
  bool bDayFoc = select == 1;
  bool bDaySel = bDayFoc && select;
  bool bMonthFoc = select == 2;
  bool bMonthSel = bMonthFoc && select;
  bool bYearFoc = select == 3;
  bool bYearSel = bYearFoc && select;
  ...
  //print days
  drawSelectable(display.getCursorX(), display.getCursorY(), now.day(), bDayFoc, bDaySel);
  display.print("/");
  //print months
  drawSelectable(display.getCursorX(), display.getCursorY(), now.month(), bMonthFoc, bMonthSel);
  display.print("/");
  //print years
...
{
  bool bHourFoc = select == 1;
  bool bHourSel = bHourFoc && select;
  bool bMinuteFoc = select == 2;
  bool bMinuteSel = bMinuteFoc && select;
  bool bSecondFoc = select == 3;
  bool bSecondSel = bSecondFoc && select;

  display.setTextColor(SH110X_WHITE);
  display.setTextSize(2);
  display.setCursor(x, y);
  
  drawSelectable(x, y, now.hour(), bHourFoc, bHourSel);
  display.print(":");
  //print minutes
  drawSelectable(display.getCursorX(), display.getCursorY(), now.minute(), bMinuteFoc, bMinuteSel);
  display.print(":");
  //print seconds
  drawSelectable(display.getCursorX(), display.getCursorY(), now.second(), bSecondFoc, bSecondSel);
}