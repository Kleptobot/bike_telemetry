...
bool LC76G::readResponse(uint8_t* buffer, uint16_t length) { return i2cRead(ADDR_READ, buffer, length); }

// High-level helper: Write data to the module
bool LC76G::writeData(const uint8_t* buffer, uint16_t length) { return i2cWrite(ADDR_WRITE, buffer, length); }
